...
// --- Assets ---
let groundImg, platformLCImg, platformRCImg, wallLImg, wallRImg;
...
// --- Control tuning ---
const MOVE_ACCEL = 0.38; // slightly less "fly"
const MOVE_FRICTION = 0.8; // a bit more damping
const MAX_RUN_SPEED = 3.2;
...
let lastVy = 0; // previous frame vertical velocity
...
// --- UI / instructions ---
let showHelp = true;
let helpTimer = 180; // 3 seconds at 60fps
...
function setup() {
  ...
  // Smaller collider for better platform feel
  player.w = 20;
  player.h = 20;
  player.friction = 0;
  player.bounciness = 0;
  ...
  // --- Platform colliders (thin) ---
  platforms = [];
  platforms.push(new Sprite(90, 120, 60, 10, "static"));
  platforms.push(new Sprite(230, 85, 60, 10, "static"));

  ground.friction = 0.8;
  for (let p of platforms) p.friction = 0.8;

  // Hide physics shapes, we draw textures manually
  ...
  // --- Coins (static sensors) ---
  resetCoins();

  // --- Exit door collider (static) ---
  exitDoor = new Sprite(width - 60, height - 40, 20, 30, "static");
  exitDoor.visible = false;

  // --- Audio setup (start muted, enable after user gesture) ---
  audioEnabled = false;
...
function draw() {
  ...
  if (!audioEnabled && anyControlPressedThisFrame()) {
    userStartAudio();
    audioEnabled = true;
  }
  ...
  // --- Help overlay timer ---
  if (showHelp) {
    if (anyControlPressedThisFrame()) showHelp = false;
    else {
      helpTimer--;
  ...
  // --- Cache previous frame values for reliable landing ---
  let prevVy = lastVy;
  let wasGrounded = lastGrounded;

  // --- Grounded check (top-only) ---
  ...
  // --- Landing detection: immediate + consistent ---
  // Use "new contact" signal (collided) OR grounded state flip.
  ...
  // Trigger only if we were actually falling previously
  if (
  ...
  // --- Prevent micro-bounce after landing ---
  if (isGrounded && player.vel.y > 0) player.vel.y = 0;
  ...
  // --- Exit activation ---
  if (coinCount >= 3) exitActive = true;

  // --- Player reaches exit (MOVED OUT of coin loop!) ---
  ...
  if (winTimer > 0) {
    ...
    if (winTimer === 1) resetLevel();
  }

  // --- Hard bounds clamp (prevents edge launch) ---
...
function anyControlPressedThisFrame() {
  return (
    kb.presses("a") ||
    kb.presses("d") ||
    kb.presses("w") ||
...
function isStandingOn(s, tolerance = 5) {
  // True only when the player is on top of the surface
...
function drawHelpOverlay() {
  ...
  textSize(10);
  text("Controls:", 36, 38);
  text("A/D or \u2190/\u2192 : Move", 36, 52);
  text("W or \u2191 : Jump", 36, 64);
  text("Space : Attack", 36, 76);
